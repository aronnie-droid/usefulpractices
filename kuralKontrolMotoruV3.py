...
def main():
    # PARAMETRELER (YENİ ARAYÜZE GÖRE SIRALANDI)
    ...
    rule_csv = arcpy.GetParameterAsText(2)   # Kurallar CSV Dosyası
    note_field = arcpy.GetParameterAsText(3) # Not Yazılacak Alan

    arcpy.AddMessage("Adım 1: Veri yeni konuma kopyalanıyor...")
    
    # Orijinal veriyi bozmamak için kopyasını oluşturuyoruz (Var olanı siler)
    if arcpy.Exists(output_fc):
        arcpy.management.Delete(output_fc)
    
    # Veri tipine göre kopyalama 
